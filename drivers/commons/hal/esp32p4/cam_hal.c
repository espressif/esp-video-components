...
int cam_hal_config_xclk(cam_hal_context_t *hal, uint32_t xclk_freq)
{
    ...
    if (src_clk_hz && !(src_clk_hz % xclk_freq)) {
        int xclk_div_num = src_clk_hz / xclk_freq;
    ...
    HAL_LOGE("cam_hal", "CLK config err src_clk_hz=%d xclk_freq=%d", (int)src_clk_hz, (int)xclk_freq);
    return -1;
}
...
int cam_hal_config_port_xclk(int port, uint32_t xclk_freq)
{
    ...
    PERIPH_RCC_ATOMIC() {
        cam_ll_enable_clk(hal.hw, true);
    };

    PERIPH_RCC_ACQUIRE_ATOMIC(cam_periph_signals[port].module, ref_count) {
        if (ref_count == 0) {
    ...
    return cam_hal_config_xclk(&hal, xclk_freq);
}
...
void cam_hal_init(cam_hal_context_t *hal, uint8_t port)
{
    ...
    cam_hal_set_vsync_filter_num(hal, 0); // Filter by LCD_CAM clock
...
{
    /* Drop data in cache and reload data from RAM */

    Cache_Invalidate_Addr(CACHE_MAP_L2_CACHE | CACHE_MAP_L1_DCACHE, (uint32_t)src, size);
    memcpy(dst, src, size);
}
