/*
 * SPDX-FileCopyrightText: 2023 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#pragma once

#include <stdint.h>
#include <stddef.h>
#include <sys/queue.h>
#include "esp_err.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * @brief Video buffer element object.
 */
struct esp_video_buffer_element {
    SLIST_ENTRY(esp_video_buffer_element) node;     /*!< List node */ 
    uint8_t buffer[];                               /*!< Buffer space to fill data */ 
};

/**
 * @brief Video buffer object.
 */
struct esp_video_buffer {
    SLIST_HEAD(esp_video_buffer_list, esp_video_buffer_element) free_list;   /*!< Free buffer elements list  */ 
    SLIST_HEAD(esp_video_buffer_list, esp_video_buffer_element) done_list;   /*!< Done buffer elements list  */

    portMUX_TYPE spinlock;                          /*!< Buffer lock */

    size_t element_count;                           /*!< Element count */ 
    size_t element_size;                            /*!< Element buffer size */ 

    uint8_t buffer[];                               /*!< Element buffer */
};

/**
 * @brief Create video buffer object.
 *
 * @param count Buffer element count
 * @param size  Buffer element size
 *
 * @return
 *      - Video buffer object pointer on success
 *      - NULL if failed
 */
struct esp_video_buffer *buffer esp_video_buffer_create(size_t count, size_t size);

/**
 * @brief Destroy video buffer object.
 *
 * @param buffer Video buffer object
 *
 * @return
 *      - ESP_OK on success
 *      - Others if failed
 */
esp_err_t esp_video_buffer_destroy(struct esp_video_buffer *buffer);

/**
 * @brief Allocate one buffer element, remove it from free list.
 *
 * @param buffer Video buffer object
 *
 * @return
 *      - Video buffer element object pointer on success
 *      - NULL if failed
 */
struct esp_video_buffer_element *esp_video_buffer_alloc(struct esp_video_buffer *buffer);

/**
 * @brief Insert one buffer element to done list, this means operation is success.
 *
 * @param buffer  Video buffer object
 * @param element Video buffer element object
 *
 * @return None
 */
void esp_video_buffer_done(struct esp_video_buffer *buffer, struct esp_video_buffer_element *element);

/**
 * @brief Free one buffer element, insert it to free list.
 *
 * @param buffer  Video buffer object
 * @param element Video buffer element object
 *
 * @return None
 */
void esp_video_free_buffer(struct esp_video_buffer *buffer, struct esp_video_buffer_element *element);

#ifdef __cplusplus
}
#endif
