/*
 * SPDX-FileCopyrightText: 2026 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */

static const sp0a39_reginfo_t SPI_1bit_24Minput_Gray_640x480_4fps[] = {
    {0xfd, 0x00},
    {0x1d, 0x25},
    {0x31, 0x05},
    {0x32, 0x01},
    {0x30, 0x11},
    {0xfd, 0x00},
    {0xf0, 0xff}, // BLC
    {0xf1, 0xff},
    {0xf2, 0xff},
    {0xf3, 0xff},
    {0xfc, 0x50}, // bpc_p
    {0xfd, 0x01},
    {0x5d, 0x01},
    {0x34, 0xe3},
    {0x35, 0x01},
    {0xfd, 0x00}, // ae setting
    {0x03, 0x00},
    {0x04, 0x78},
    {0x24, 0x40},
    {0xef, 0x40},
    {0x06, 0x00},
    {0x09, 0x00},
    {0x0a, 0x46},
    {0x10, 0x0a}, // 07  by wwj 0124
    {0x11, 0x02}, // 04  by wwj 0124
    {0x16, 0x01},
    {0x19, 0x22},
    {0x1e, 0x60}, // 58 by wwj 0124
    {0x29, 0x60}, // 48 by wwj 0124
    {0x13, 0x37},
    {0x14, 0x01},
    {0x25, 0x01},
    {0x2a, 0x06},
    {0x27, 0x00}, // by wwj 0218
    {0x54, 0x00},
    {0x55, 0x10},
    {0x58, 0x28}, // 38 by wwj 0124
    {0x5d, 0x12},
    {0x63, 0x00},
    {0x64, 0x00},
    {0x66, 0x2a}, // 28   by wwj 0218
    {0x68, 0x28}, // 2a   by wwj 0218
    {0x72, 0x32}, // 3a by wwj 0124
    {0x73, 0x0a},
    {0x75, 0x30}, // 48 by wwj 0124
    {0x76, 0x0a},
    {0x1f, 0x99}, // 77  by wwj 0215
    {0x20, 0x09}, // 7 by wwj 0215
    {0xfb, 0x16},
    {0xfd, 0x01},
    {0xf2, 0x69},
    {0xf7, 0x14}, // base
    {0x02, 0x19}, // max base
    {0x03, 0x01}, // min base
    {0x06, 0x14}, // max outdoor
    {0x08, 0x01}, // min exp outdoor
    {0xfd, 0x02}, // ae gain &status
    {0xb8, 0x50}, // nr to dummy mean
    {0xb9, 0xff}, // dummy to nr mean
    {0xba, 0x40}, // dummy to low  mean
    {0xbb, 0x45}, // low to dummy  mean
    {0xbc, 0xc0}, // dummy to low
    {0xbd, 0x50}, // nr to dummy rpc
    {0xbe, 0xF4}, // nr to dummy exp
    {0xbf, 0x01},
    {0xd0, 0xF4}, // dummy to low exp
    {0xd1, 0x01},
    {0xfd, 0x01}, // rpc
    {0xc0, 0x1f}, // rpc_1base_max
    {0xc1, 0x18}, // rpc_2base_max
    {0xc2, 0x15}, // rpc_3base_max
    {0xc3, 0x13}, // rpc_4base_max
    {0xc4, 0x13}, // rpc_5base_max
    {0xc5, 0x12}, // rpc_6base_max
    {0xc6, 0x12}, // rpc_7base_max
    {0xc7, 0x11}, // rpc_8base_max
    {0xc8, 0x11}, // rpc_9base_max
    {0xc9, 0x11}, // rpc_10base_max
    {0xca, 0x10}, // rpc_11base_max
    {0xf3, 0x10}, // rpc_12base_max
    {0xf4, 0x10}, // rpc_13base_max
    {0xfd, 0x01}, // ae min gain
    {0x04, 0xff}, // rpc_max_indr
    {0x05, 0x10}, // rpc_min_indr
    {0x0a, 0x30}, // rpc_max_outdr
    {0x0b, 0x10}, // rpc_min_outdr
    {0xfd, 0x01}, // target
    {0xcb, 0x38}, // target indr
    {0xcc, 0x35}, // target outdr
    {0xcd, 0x08}, // lock range
    {0xce, 0x0F}, // hold range
    {0xfd, 0x00},
    {0xfb, 0x16},
    {0x35, 0xaa},
    {0xfd, 0x01}, // lsc
    {0x1e, 0x00},
    {0x20, 0x00},
    {0x84, 0x25},
    {0x85, 0x25},
    {0x86, 0x1f},
    {0x87, 0x23},
    {0x88, 0x1c},
    {0x89, 0x20},
    {0x8a, 0x1a},
    {0x8b, 0x15},
    {0x8c, 0x15},
    {0x8d, 0x1a},
    {0x8e, 0x0a},
    {0x8f, 0x13},
    {0x90, 0x13},
    {0x91, 0x00},
    {0x92, 0x0a},
    {0x93, 0x08},
    {0x94, 0x12},
    {0x95, 0x00},
    {0x96, 0x0a},
    {0x97, 0x08},
    {0x98, 0x15},
    {0x99, 0x00},
    {0x9a, 0x0a},
    {0x9b, 0x05},
    {0xe8, 0x20},
    {0xe9, 0x0f},
    {0xea, 0x00},
    {0xbd, 0x1e},
    {0xbe, 0x00},
    {0xfd, 0x01},
    {0xa4, 0x00}, // lum_limit
    {0x0e, 0x80}, // heq_mean  outdoor,indoor
    {0x18, 0x80}, // heq_mean  dummy,lowlight
    {0x0f, 0x20}, // k_max
    {0x10, 0x90}, // ku_outdoor
    {0x11, 0x80}, // ku_nr
    {0x12, 0x80}, // ku_dummy
    {0x13, 0xa0}, // ku_low
    {0x14, 0x80}, // kl_outdoor
    {0x15, 0x90}, // kl_nr
    {0x16, 0x85}, // kl_dummy
    {0x17, 0x85}, // kl_low
    {0x6e, 0x00}, // gamma
    {0x6f, 0x03},
    {0x70, 0x07},
    {0x71, 0x0d},
    {0x72, 0x17},
    {0x73, 0x29},
    {0x74, 0x3d},
    {0x75, 0x4f},
    {0x76, 0x5f},
    {0x77, 0x79},
    {0x78, 0x8c},
    {0x79, 0x9d},
    {0x7a, 0xa9},
    {0x7b, 0xb3},
    {0x7c, 0xbe},
    {0x7d, 0xc7},
    {0x7e, 0xd0},
    {0x7f, 0xd6},
    {0x80, 0xde},
    {0x81, 0xe4},
    {0x82, 0xe9},
    {0x83, 0xff},
    {0xfd, 0x02}, // skin detect
    {0x09, 0x06},
    {0x0d, 0x1a},
    {0x1c, 0x09},
    {0x1d, 0x03},
    {0x1e, 0x10}, // awb
    {0x1f, 0x06},
    {0xfd, 0x01},
    {0x32, 0x00},
    {0xfd, 0x02},
    {0x26, 0xcb},
    {0x27, 0xc2},
    {0x10, 0x00}, // br offset
    {0x11, 0x00}, // br offset_f
    {0x18, 0x17},
    {0x19, 0x36},
    {0x2a, 0x01},
    {0x2b, 0x10},
    {0x28, 0xf8},
    {0x29, 0x08},
    {0x66, 0x5F}, // d65 10
    {0x67, 0x7f},
    {0x68, 0xE0},
    {0x69, 0x10},
    {0x6a, 0xa6},
    {0x7c, 0x4A}, // indoor 11
    {0x7d, 0x80},
    {0x7e, 0x00},
    {0x7f, 0x30},
    {0x80, 0xaa},
    {0x70, 0x32}, // cwf 12
    {0x71, 0x60},
    {0x72, 0x30},
    {0x73, 0x5a},
    {0x74, 0xaa},
    {0x6b, 0xff}, // tl84 13
    {0x6c, 0x50},
    {0x6d, 0x40},
    {0x6e, 0x60},
    {0x6f, 0x6a},
    {0x61, 0xff}, // f 14
    {0x62, 0x27},
    {0x63, 0x51},
    {0x64, 0x7f},
    {0x65, 0x6a},
    {0x75, 0x80},
    {0x76, 0x09},
    {0x77, 0x02},
    {0x0e, 0x12},
    {0x3b, 0x09},
    {0x48, 0xea}, // green u center
    {0x49, 0xfc}, // green v center
    {0x4a, 0x05}, // green range
    {0x02, 0x00}, // outdoor awb exp 5msb
    {0x03, 0x88}, // outdoor awb exp 8lsb
    {0xf5, 0xfe}, // a8 ;outdoor awb rgain top
    {0x22, 0xfe}, // sharp
    {0x20, 0xfe},
    {0xf7, 0xfe},
    {0xfd, 0x02},
    {0xde, 0x00},
    {0xcf, 0x0a}, // zy
    {0xd7, 0x0a},
    {0xd8, 0x12},
    {0xd9, 0x14},
    {0xda, 0x1a},
    {0xdc, 0x07},
    {0xe8, 0x60}, //ֵΧ
    {0xe9, 0x40},
    {0xea, 0x40},
    {0xeb, 0x30},
    {0xec, 0x60}, //ֵΧ
    {0xed, 0x50},
    {0xee, 0x40},
    {0xef, 0x30},
    {0xd3, 0x30}, // Ե񻯷ΧԵֵԽ񻯷ΧԽ
    {0xd4, 0xc0}, //
    {0xd5, 0x50}, // 񻯵СֵΧ
    {0xd6, 0x0b},
    {0xf0, 0x7f},
    {0xfd, 0x01}, // skin sharpen
    {0xb1, 0xf0},
    {0xfd, 0x02},
    {0xdc, 0x07},
    {0x05, 0x08},
    {0xfd, 0x01}, // bpc
    {0x26, 0x33},
    {0x27, 0x99},
    {0x62, 0xf0},
    {0x63, 0x80},
    {0x64, 0x80},
    {0x65, 0x20},
    {0xfd, 0x02}, // dns
    {0xdd, 0x00},
    {0xfd, 0x01},
    {0xa8, 0x00}, // ͨƽֵ
    {0xa9, 0x09},
    {0xaa, 0x09},
    {0xab, 0x0c},
    {0xd3, 0x00}, // GrGbƽֵ
    {0xd4, 0x09},
    {0xd5, 0x09},
    {0xd6, 0x0c},
    {0xcf, 0xff}, // Gr\Gb֮ƽǿ
    {0xd0, 0xf0},
    {0xd1, 0x80},
    {0xd2, 0x80},
    {0xdf, 0xff}, // GrGbͨƽǿ
    {0xe0, 0xf0},
    {0xe1, 0xd0},
    {0xe2, 0x80},
    {0xe3, 0xff}, // RBƽǿ
    {0xe4, 0xf0},
    {0xe5, 0xd0},
    {0xe6, 0x80},
    {0xfd, 0x02}, // CCM
    {0x15, 0xe0}, // b>th
    {0x16, 0x95}, // r<th
    {0xa0, 0x9b}, // !F
    {0xa1, 0xe4},
    {0xa2, 0x01},
    {0xa3, 0xf2},
    {0xa4, 0x8f},
    {0xa5, 0xff},
    {0xa6, 0x01},
    {0xa7, 0xdb},
    {0xa8, 0xa4},
    {0xac, 0x80}, // F
    {0xad, 0x21},
    {0xae, 0xdf},
    {0xaf, 0xf2},
    {0xb0, 0xa0},
    {0xb1, 0xee},
    {0xb2, 0xea},
    {0xb3, 0xd9},
    {0xb4, 0xbd},
    {0xfd, 0x01}, // sat u B
    {0xb3, 0xb0},
    {0xb4, 0x90},
    {0xb5, 0x70},
    {0xb6, 0x55},
    {0xb7, 0xb0}, // sat v R
    {0xb8, 0x90},
    {0xb9, 0x70},
    {0xba, 0x55},
    {0xfd, 0x01}, // auto_sat
    {0xbf, 0xff}, // y_mean_th
    {0x00, 0x00}, // status
    {0xfd, 0x01}, // low_lum_offset
    {0xa4, 0x00}, // lum_limit
    {0xa5, 0x1f}, // lum_set
    {0xa6, 0x50}, // black vt
    {0xa7, 0x65}, // white vt
    {0xfd, 0x02}, // gw
    {0x30, 0x38}, // 38 ;00  guan gw_en
    {0x31, 0x40},
    {0x32, 0x40},
    {0x33, 0xd0},
    {0x34, 0x10},
    {0x35, 0x60},
    {0x36, 0x28},
    {0x37, 0x07},
    {0x38, 0x08}, // (read 0x31_gw_offset>0x10,is gray image)
    {0xe6, 0x8F}, // (bit7:4 white edge 3:0 dark edg zhiyuedafanweiyueda )
    {0xfd, 0x01},
    {0x1b, 0x15}, // (baibanshiku&kljiaduoshao)
    {0x1c, 0x1A}, // (Read 0x31*0x1cֵ/16Ϊռӵֵ)
    {0x1d, 0x0c}, // auto contrast enable ,bit3 outdoor  bit2 indoor bit1 dummy  bit0 lowlight
    {0xfd, 0x01},
    {0x32, 0x15},
    {0x33, 0xef},
    {0x36, 0x10}, // AEͳGAmma֮ǰ00AEGamma֮
    {0xf6, 0xb0}, // ̬ƣֵԽسӰС
    {0xf5, 0x10}, // ֹ̬ɫ뵼¹أ
    {0xd7, 0x3a}, // tbh
    {0xd8, 0x10},
    {0xd9, 0x20},
    {0xda, 0x10},
    {0xdb, 0x7a},
    {0xdc, 0x3a},
    {0xdd, 0x30},
    {0xde, 0x30},
    {0xe7, 0x3a},
    {0x9c, 0xaa}, // uv_th
    {0x9d, 0xaa},
    {0x9e, 0x55},
    {0x9f, 0x55},
    {0xfd, 0x00},
    {0x1c, 0x00},
    {0xfd, 0x00},
    // y-out en
    {0xfd, 0x00},
    {0x31, 0x00}, // PCLK idle stats
    {0x30, 0x11},
    {0x34, 0x04}, // [2]y_out_en
    {0xfd, 0x01},
    {0x5d, 0x01},
    {0x32, 0x05},
    {0x35, 0x04},
    {0xfd, 0x02},
    {0x26, 0x80},
    {0x27, 0x80},
    // MTK SDR1BIT
    {0xfd, 0x00},
    {0x30, 0x1c},
    {0x1c, 0x9e}, // [6]out_vsync
    {0x2c, 0x1d},
    {0x2e, 0xf0},
    {0xfd, 0x00},
};
