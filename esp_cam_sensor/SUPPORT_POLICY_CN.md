...
为保障图像质量（IQ）调试工作的顺利开展，请在提交调试请求前准备并确认以下信息：

### 图像质量调试基础信息表

| 信息类别 | 所需细节说明 |
| :--- | :--- |
| **镜头参数** | 焦距、视场角（FOV）、光圈值（F-Number）、主光线角（CRA）曲线，以及红外（IR）滤光片规格（如 650nm / 850nm / 940nm）。 |
| **补光灯参数（选填）** | 补光灯类型（白光 / 850nm 红外 / 940nm 红外）、驱动方式（PWM 调光 / 电平开关）、最大功率及发散角。 |
| **显示器参数（选填）** | 显示屏型号、面板材质（OLED / LCD）、屏幕 Gamma 曲线及所采用的色彩空间标准（如 sRGB）。 |
| **光敏传感器参数（选填）**| 光敏传感器的具体芯片型号。 |
| **IR-CUT 参数（选填）** | IR-CUT 驱动芯片型号，以及 IR-CUT 与补光灯的软硬件联动机制。 |
| **对焦马达参数（选填）** | 自动对焦（AF）马达的芯片型号。 |

### 提报须知与注意事项

* **模组状态确认**：鉴于镜头光学特性对最终成像质量具有决定性影响，请用户务必在**明确相机模组的视场角、对焦距离，并评估其基础噪声表现**后，再行提交调试申请。
* **标样管理（Golden Sample）**：为确保调试后的图像效果在量产中具备良好的兼容性与一致性，请用户将选定的**标准样机（Golden Sample）**寄送至项目经理处，以此作为后续多批次模组画质验收的基准参照物。
