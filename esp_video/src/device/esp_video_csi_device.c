...
static esp_err_t init_config(struct esp_video *video)
{
    ...
    ESP_RETURN_ON_ERROR(csi_get_data_lane(sensor_format.mipi_info.lane_num, &csi_video->csi_data_lane_num), TAG, "failed to get CSI data lane number");
    ESP_RETURN_ON_ERROR(csi_get_input_frame_type(sensor_format.format, &csi_video->csi_in_color, &csi_in_bpp), TAG, "failed to get CSI input frame format");

    csi_video->lane_bit_rate_mbps = sensor_format.mipi_info.mipi_clk / (1000 * 1000);

    if (sensor_format.isp_info) {
        csi_video->bypass_isp = false;
...
static esp_err_t csi_video_start(struct esp_video *video, uint32_t type)
{
    ...
    ESP_GOTO_ON_ERROR(esp_video_isp_start(csi_video->bypass_isp, csi_video->csi_in_color, csi_video->csi_out_color,
                                          csi_video->csi_line_sync, csi_config.h_res, csi_config.v_res),
...
exit_3:
    esp_video_isp_stop(csi_video->bypass_isp);
exit_2:
...
static esp_err_t csi_video_stop(struct esp_video *video, uint32_t type)
{
    ...
    ESP_RETURN_ON_ERROR(esp_video_isp_stop(csi_video->bypass_isp), TAG, "failed to stop ISP");
...
static esp_err_t csi_video_set_format(struct esp_video *video, uint32_t type, const struct esp_video_format *format)
{
    ...
    if (csi_video->bypass_isp) {
        ...
        bool isp_support_fmt = false;
        uint32_t index = 0;
        uint32_t isp_format;
        while (esp_video_isp_enum_format(index, &isp_format) == ESP_OK) {
            if (isp_format == format->pixel_format) {
                isp_support_fmt = true;
                break;
            }
            index++;
        }
        ESP_RETURN_ON_FALSE(isp_support_fmt, ESP_ERR_NOT_SUPPORTED, TAG, "ISP does not support format=%" PRIx32, format->pixel_format);

        ESP_RETURN_ON_ERROR(csi_get_output_frame_type_from_v4l2(format->pixel_format, &csi_video->csi_out_color, &csi_video->csi_out_bpp),
                            TAG, "CSI does not support format=%" PRIx32, format->pixel_format);
