...
.. attention::
    ...
    esp_cam_sensor 组件中的接口主要是相机驱动的底层开发人员进行开发和维护，应用开发人员可以直接使用 esp-video 的 API 接口，避免直接使用 esp_cam_sensor 组件中的接口。
...
相机设备的格式信息包括：输出接口、输出的数据的格式、输出的帧率等信息。所有由初始化寄存器列表决定的信息均通过 ``esp_cam_sensor_format_t`` 结构体来描述。通过设置和查询该结构体的信息，接收端才能正确地完成初始化操作，接收和处理相机发送的图像数据。
...
.. attention::
    ...
    不同的外设可以使用的时钟源是不一样的，比如一些芯片的 LEDC 外设只能使用 80M 大小的时钟源，由于该时钟源的频率无法整除 24M，因此无法生成 24M 的 XCLK。如果不确定是否可以生成指定大小的时钟信号，请联系 FAE。

Kconfig 选项
^^^^^^^^^^^^^^^^

每一个传感器都有一个配置文件，以 OV2710 为例，对应的是 ``sensors/ov2710/Kconfig.ov2710`` 文件。该配置文件可以配置传感器默认加载的格式、以及是否启用该传感器的驱动等信息。
